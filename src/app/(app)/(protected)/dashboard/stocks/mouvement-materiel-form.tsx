...
import { ajouterMouvementMateriel } from "@/lib/actions/stocks";
import { CATEGORIES_BESOIN, CATEGORIE_BESOIN_LABELS } from "@/lib/categorie-besoin";
...
export function MouvementMaterielForm({
  maraudes,
}: {
  maraudes: { id: string; date_heure: string }[];
}) {
  const [state, action, pending] = useActionState(ajouterMouvementMateriel, undefined);
  ...
  return (
    <form ref={formRef} action={action} className="flex flex-col gap-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-2">
          ...
          <Select name="categorie" required>
            <SelectTrigger id="categorie" className="h-12">
              <SelectValue placeholder="Choisir…" />
            </SelectTrigger>
            <SelectContent>
              {CATEGORIES_BESOIN.map((c) => (
                <SelectItem key={c} value={c}>
                  {CATEGORIE_BESOIN_LABELS[c]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Select name="type" defaultValue="entree" required>
            ...
            <SelectContent>
              <SelectItem value="entree">Entrée (don reçu)</SelectItem>
              <SelectItem value="sortie">Sortie (distribué)</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Input id="motif" name="motif" placeholder="Ex. « Don association X »" className="h-12" />
      </div>
